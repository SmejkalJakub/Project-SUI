...
do
    ...
    sed -i "s/            player_areas_weight = .*/            player_areas_weight = $v/" dicewars/ai/xgrunw00/xgrunw00.py
    sed -i "s/            biggest_region_weight = .*/            biggest_region_weight = $w/" dicewars/ai/xgrunw00/xgrunw00.py
    sed -i "s/            weak_borders_weight = .*/            weak_borders_weight = $x/" dicewars/ai/xgrunw00/xgrunw00.py
    sed -i "s/            weak_enemies_weight = .*/            weak_enemies_weight = $y/" dicewars/ai/xgrunw00/xgrunw00.py
    
    sed -i "s/if(hold_prob >= 0\..* and succ_prob >= 0\..*):/if(hold_prob >= $hold_prob and succ_prob >= $succ_prob):/" dicewars/ai/xgrunw00/xgrunw00.py
