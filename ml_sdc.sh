...
do
    ...
    sed -i "s/            v = .*/            v = $v/" dicewars/ai/xsmejk28_sdc.py
    sed -i "s/            w = .*/            w = $w/" dicewars/ai/xsmejk28_sdc.py
    sed -i "s/            x = .*/            x = $x/" dicewars/ai/xsmejk28_sdc.py
    sed -i "s/            y = .*/            y = $y/" dicewars/ai/xsmejk28_sdc.py

    sed -i "s/if(hold_prob >= 0\..* and succ_prob >= 0\..*): #tohle je zatim asi nejlepsi podminka/if(hold_prob >= $hold_prob and succ_prob >= $succ_prob): #tohle je zatim asi nejlepsi podminka/" dicewars/ai/xsmejk28_sdc.py


    for i in $(seq 1 $itterations)
    do
        result=$(python3 ./scripts/dicewars-ai-only.py -r -b 11 -o 22 -s 33 -c 44 -n $itterations -l ../logs --ai dt.sdc xsmejk28_sdc)
        result_value=${result#*\'xsmejk28_sdc \(AI\)\':}
